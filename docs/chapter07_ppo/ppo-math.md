...
PPO-Clip 代理目标 $J^{\text{CLIP}}(\theta)$ 是对 $J(\theta)$ 的近似，所以优化方向同样是**最大化**这个目标。

但 PyTorch 优化器的工作方式是**最小化** loss——`loss.backward()` 计算的是 loss 减小的方向。为了用 PyTorch 的优化器实现"最大化目标"，最直接的做法是把目标取负号，变成 loss：

$$
L^{\text{CLIP}}(\theta) = -J^{\text{CLIP}}(\theta)
$$

优化器最小化这个 loss，等价于最大化 $J^{\text{CLIP}}(\theta)$。
