...
$$
\mathcal{L}(\theta) = \mathbb{E}\left[\left(r + \gamma (1-d) \max_{a'} Q(s', a'; \theta^-) - Q(s, a; \theta)\right)^2\right] \quad \text{（深度 Q 网络损失函数：TD Target 与当前 Q 值的均方误差）}
$$

> **深度 Q 网络损失函数（DQN Loss）：**
>
> - $Q(s, a; \theta)$：Q 网络对当前状态-动作对的 Q 值估计，网络参数为 $\theta$。
> - $r + \gamma (1-d) \max_{a'} Q(s', a'; \theta^-)$：TD Target，其中 $\theta^-$ 是目标网络的参数（不参与梯度更新，定期从 Q 网络复制），$d$ 表示是否真正终止。
...
class DQNAgent:
    ...
    def update(self):
        ...
        # 计算 TD Target：r + γ(1-done) max Q(s', a'; θ⁻)
        with torch.no_grad():
